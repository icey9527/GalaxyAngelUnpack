...
def extract_str(input_file: str, code_file: str, out_file: str) -> None:
    ...
    for addr, op, arg in code:
        ...
        if op == "CALL_FUN_00242E50":
            ...
            if arg == "0x300000013" or arg == "0x400000013":
                语音 = 临时表[len(临时表)-2][2]
            if arg == "0x400000013":
                语音 = 临时表[len(临时表)-3][2]

            if arg == "0x100000000" and 语音 :
                voice_num +=1
                文本指针 = 临时表[len(临时表)-2][2]
                地址 = 临时表[len(临时表)-2][0]
                print(input_file)
                voice.append((voice_num, str(语音) + '.wav', extract_shift_jis(str_data, 文本指针), 说话人))
                文本数[hex(文本指针)] = []
                语音 = None
...
def generate_target_paths(asb_dir, txt_dir, csv_dir):
    ...
    with open("voice.csv", "w+", encoding='utf-8', newline='', errors='ignore') as f:
        writer = csv.writer(f)
        for item in voice:
            if len(item) >= 3:  # 确保每行至少有3个项
                # 将第二个项用引号包裹
